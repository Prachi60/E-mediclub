...
export default function StatsCard({ title, value, change, isPositive = true, icon: Icon, color = 'teal', sparklineData = [10, 20, 15, 30, 25, 40] }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className={`glass-card rounded-[28px] border border-slate-100/60 p-5 shadow-premium bg-gradient-to-br ${activeColor.gradient} hover:shadow-premium-hover transition-all duration-300 relative overflow-hidden flex flex-col justify-between`}
    >
      <div className="flex items-start justify-between">
        ...
        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center border ${activeColor.light}`}>
          <Icon className="text-xl shrink-0" />
        </div>
      </div>

      <div className="flex items-center justify-between mt-5 pt-3 border-t border-slate-100/40">
        {/* Percentage trend indicators */}
        <div className="flex items-center gap-1">
          {isPositive ? (
            <span className="flex items-center gap-0.5 text-[10px] font-black text-teal bg-teal-light px-2 py-0.5 rounded-full uppercase">
              <FiTrendingUp className="text-xs" /> {change}
            </span>
          ) : (
            <span className="flex items-center gap-0.5 text-[10px] font-black text-coral bg-coral-light px-2 py-0.5 rounded-full uppercase">
              <FiTrendingDown className="text-xs" /> {change}
            </span>
          )}
          <span className="text-[9px] text-slate-400 font-extrabold uppercase ml-1">vs last month</span>
        </div>
